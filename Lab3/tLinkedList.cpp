...
TNode::TNode( const shared_ptr<Trapezoid> t ) {
  this->t = make_shared<Trapezoid>();
  this->t = t;
  next = NULL;
}
...
TNode::TNode( const Trapezoid & t ) {
  this->t = make_shared<Trapezoid>();
  this->t = shared_ptr<Trapezoid> ( new Trapezoid( t ) );
  next = NULL;
}
...
shared_ptr<Trapezoid> & TNode::val() {
  return t;
}
...
const shared_ptr<Trapezoid> TLinkedList::First() {
  if ( len == 0 ) {
...
const shared_ptr<Trapezoid> TLinkedList::Last() {
  if ( len == 0 ) {
...
const shared_ptr<Trapezoid> TLinkedList::GetItem( size_t idx ) {
  if ( idx >= len ) {
...
void TLinkedList::InsertFirst( const shared_ptr<Trapezoid> trapezoid ) {
...
void TLinkedList::InsertLast( const shared_ptr<Trapezoid> trapezoid ) {
  TNode * newNode = new TNode( trapezoid );
  if ( len == 0 ) {
    first = newNode;
    len++;
...
void TLinkedList::Insert( const shared_ptr<Trapezoid> trapezoid, size_t position ) {
  if ( position > len ) {
